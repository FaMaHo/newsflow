...
Falls back to extractive summarization if GROQ_API_KEY is not set or unreachable.
"""
...
class AIEngine:
    ...
    async def ping(self) -> bool:
        """Check if Groq is reachable with a minimal test call."""
        if not self._client:
            return False
        try:
            ...
            log.warning("Groq ping failed: %s", e)
            return False
